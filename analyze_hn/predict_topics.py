...
def load_article(label, story_id):
    article_id = story_id.item()
    try:
        article = Article.objects.get(hn_id=article_id)
    except Article.DoesNotExist:
        print("Article " + str(article_id) + " not found, fetching")
        article = ArticleFetcher().fetch_single(article_id)

    if article is None:
        print("Skipping article: " + str(article_id))
        return 0
